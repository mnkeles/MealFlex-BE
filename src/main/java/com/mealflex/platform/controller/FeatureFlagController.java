...
@RestController @RequiredArgsConstructor public class FeatureFlagController {private final FeatureFlagService service;
 ...
 @GetMapping("/v1/admin/feature-flags") public List<FeatureFlag> all(){return service.list();}
 @PutMapping("/v1/admin/feature-flags/{key}") public FeatureFlag set(@PathVariable String key,@RequestBody Map<String,Object> body){return service.upsert(key,(String)body.getOrDefault("description",""),Boolean.TRUE.equals(body.get("enabled")),((Number)body.getOrDefault("rolloutPercent",0)).intValue());}
}
